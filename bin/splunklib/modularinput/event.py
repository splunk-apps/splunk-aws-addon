...
class Event(object):
    ...
    def write_to(self, stream):
        ...
        This Function cannot deal with non-ascii encodings.
        In such cases, try get_element() instead.
    ...
    def get_element(self):
        """Get an XML representation of self, an ``Event`` object, and return the XML element

        The XML element will only be written if its data field is defined,
        otherwise a ``ValueError`` is raised.
        """
        if self.data is None:
            raise ValueError("Events must have at least the data field set to be written to XML.")
        ...
        if not isinstance(self.data, unicode):
            ...
            if not decode_success:
                ...
                raise UnicodeDecodeError('Data encoding is not supported.')

        element = ET.Element("event")
        if self.stanza is not None:
            element.set("stanza", self.stanza)
        element.set("unbroken", str(int(self.unbroken)))

        # if a time isn't set, let Splunk guess by not creating a <time> element
        if self.time is not None:
            ET.SubElement(element, "time").text = str(self.time)
        ...
        for node, value in subelements:
            if value is not None:
                ET.SubElement(element, node).text = value

        if self.done is not None and self.done:
            ET.SubElement(element, "done")
